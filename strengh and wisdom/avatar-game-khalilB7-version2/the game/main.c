...
int main(void)
{
    ...
    TTF_Font *font = TTF_OpenFont("images/arial.ttf", 22);
    if (!font)
    ...
    int groundY = SCREEN_HEIGHT - 72 - 10;
        ...
        while (SDL_PollEvent(&ev))
        {
            ...
            if (ev.type == SDL_KEYDOWN)
            {
                ...
                if (!players[0].dead) {
                    if (k == SDLK_k) {
                        shoot(&players[0]);
                    } else if (k == SDLK_m && players[0].state != STATE_DEAD) {
                ...
                if (numPlayers > 1 && !players[1].dead) {
                    if (k == SDLK_l) {
                        shoot(&players[1]);
                    } else if (k == SDLK_c && players[1].state != STATE_DEAD) {
