...
void initializePlayer(Hero *hero, int id, SDL_Renderer *renderer)
{
    ...
    /* Stagger starting X positions – y will be corrected by setHeroBounds/groundY */
    ...
    hero->scoreActive = 0;   /* caller must enable; avoids P2 scoring before joining */
...
/* Public wrapper so main.c can trigger state transitions correctly */
