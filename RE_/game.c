...
int initGame(Game *g, SDL_Renderer *renderer)
{
    ...
    g->platforms[0].texture = NULL; // No image

    // Player 1 (Blue Rectangle -> Now an Image)
    g->platforms[1].rect = (SDL_Rect){200, 300, 50, 50};
    g->platforms[1].type = 1;
    // Load your sprite here! Make sure "kevin.png" is in your project folder.
    g->platforms[1].texture = IMG_LoadTexture(renderer, "stand.png"); 

    // Player 2 (Red)
    g->platforms[2].rect = (SDL_Rect){0, 500, 50, 50};
    g->platforms[2].type = 2;
    g->platforms[2].texture = NULL; // No image yet
...
}void renderGame(Game *g, SDL_Renderer *renderer)
{
    ...
    for(int i = 0; i < g->nbPlatforms; i++) {
        ...
        // Make sure you use g->camera1.x for the first viewport, 
        // and g->camera2.x for the second viewport!
        p.x -= g->camera.x; 
        p.y -= g->camera.y;

        // NEW LOGIC: Check if we have an image
        if (g->platforms[i].texture != NULL) {
            
            SDL_RenderCopy(renderer, g->platforms[i].texture, NULL, &p);
        } 
    }
